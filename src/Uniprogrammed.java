...
public class Uniprogrammed {
	...
	public Uniprogrammed(ArrayList<Process> all_processes,Boolean verbose) {
	...
	{	
		...
		{	if(this.verbose)
			...
			{
				if(!(current_process.state).equals("blocked"))
						{	
							...
							if(current_process.rcpu==0)
							{
								current_process.state="terminated";
								current_process.finish_time=cycle;
								if ((current_process.state).equals("terminated")){
			...
			if(ready_processes.size()>0)
			{
					if(current_process==null)
					{
						temp=ready_processes.remove(0);
						temp.gen_CPU_burst(udri);
						temp.state="running";
			...
			{	
				Process p=it.next();
					...
					temp=blocked_processes.remove(0);
					temp.riob=(temp.riob-1);
					if (temp.riob==0){
						temp.gen_CPU_burst(udri);
						temp.state="running";
						current_process=temp;
